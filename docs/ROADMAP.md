...
17. **Node hover flicker on graph canvas** — Hovering some nodes produces a visible flicker / re-trigger of the hover state instead of the smooth crossfade introduced in caveat #11. Likely causes to investigate, in order: (a) hover state owned by a parent that re-renders on `pointermove`, recreating the `<circle>` and restarting the CSS transition; (b) overlapping SVG elements (label `<text>`, halo, edge endpoints) intercepting `pointerleave`/`pointerenter` events and toggling state; (c) the CSS transition targeting a property that also changes due to layout (e.g. `r` recomputed from a signal on every frame). Reproduce with slow `pointermove` near node edges before deciding the fix.
