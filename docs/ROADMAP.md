...
- [ ] **Security & Content Trust Baseline** _(pre-embed / pre-blob / pre-AI)_
    ...
    - **Audit follow-up 2026-05-09 (additions, not done):**
      - Webhook task supervision: `handle_push` / `handle_item_event` oggi sono `tokio::spawn` fire-and-forget; un panic nel rebuild scompare in tracing senza visibilità operativa. Avvolgere in `JoinSet` o supervisor task e loggare i panic esplicitamente. Si lega al "Background job/outbox" della Failure-Mode Matrix.
      - Schema-evolution helper tipato: `add_column_if_missing` in `server/projection/migrations.rs` costruisce DDL via `format!` con `table`/`column`/`spec` come `&str`. Oggi i call site sono costanti interne, ma il pattern accetta silenziosamente input non validato. Sostituire con un enum di tabelle ammesse + helper che restituisce DDL tipato. Va mano nella mano con "Versionare le migrazioni SQLite" di "Projection Schema v2".
      - Input size caps server-side per ogni mutazione: `SaveBrainFile` accetta body Markdown di lunghezza arbitraria → CPU-DoS via `pulldown_cmark`. Definire una policy unica `MAX_MARKDOWN_BYTES` / `MAX_FRONTMATTER_BYTES` / `MAX_PATH_LEN` e applicarla in tutte le `#[server]` fn mutate-only, coerente col `MAX_ASSET_BYTES` già esistente.
      - Rate-limit baseline: nessun limit oggi su `/auth/callback`, `/webhook/github`, o sulle server fn mutating. Introdurre `tower_governor` (o limiter hand-rolled SQLite-backed se serve persistenza) prima di esporre l'app a contributor non-org o di shippare embedded analytics.
      - Cifratura at-rest dei token OAuth nella session store: `tower-sessions` gira oggi senza chiavi `with_signed`/`with_private`. Il file SQLite di sessione contiene token `gho_*` in chiaro. Decisione binaria: (a) abilitare `with_private` e gestire la chiave (env var, rotation), oppure (b) tenere i token solo in memoria e re-derivarli alla scadenza. Va deciso insieme al posture `OAuth user / App installation / MCP` sopra.
...
- [ ] **`detail_panel.rs` split** _(audit follow-up 2026-05-09 — promote da "next candidate" a slice esplicita)_
    - Stato: `crates/brain-app/src/knowledge/detail_panel.rs` è 1199 LoC con ~14 funzioni, 70+ `.clone()`, ~10 `RwSignal<String>` per stato UI transitorio (`rename_status`, `delete_msg`, `delete_error`, `rename_input`, `delete_prompt`, `renaming`, `deleting`, …). Già riconosciuto dalla 3.7F come "next candidate" dopo la split di `editor.rs`, ma senza un trigger esplicito.
    - Direzione: stessa pattern di `editor/{mod,frontmatter,location,markdown,related,tags}.rs` — estrarre `work_item_card.rs`, `comments.rs`, `delete_dialog.rs`, `rename_dialog.rs`, `backlinks.rs` come componenti separati che ricevono signal/handler dal `mod.rs` ridotto a layout + selection routing. Niente nuovi state container globali: gli `RwSignal<String>` transitori migrano dentro i componenti che li possiedono.
    ...
    - Success criterion: `detail_panel/mod.rs` < 300 LoC; ogni sub-component testabile in isolamento; nessun behavioral change visibile a Andrea/Matteo/contributor che usa Brain UI. Verifica come per `editor.rs`: `cargo check --features ssr/hydrate` + `cargo test --features ssr` + smoke nav.
...
21. **Frontmatter round-trip preserves keys, not formatting** — _emerged from audit 2026-05-09._ `merge_frontmatter` overlay parses YAML via `serde_yaml::from_str` into `BTreeMap<String, serde_yaml::Value>` and re-serializes on save (vedi [crates/brain-domain/src/frontmatter.rs](../crates/brain-domain/src/frontmatter.rs)). I valori dei campi custom sopravvivono ai save (questo è il fix originale di 2026-04-22), ma `BTreeMap` ordina le chiavi alfabeticamente e `serde_yaml` non preserva commenti, blank line o quoting style. Conseguenza pratica: un utente che apre `.brain-config.yml` o un work item Markdown nel proprio IDE, aggiunge un commento `# state: questa è una nota`, e poi salva via Brain UI, vede il commento sparire. Non è un regression bug — è il comportamento del parser. Documenta la limitazione finché un eventuale 4.x non introduce un patch lens YAML-aware (es. `yaml-rust2` o `serde_yaml_ng`) che muta solo il sotto-albero che cambia.
