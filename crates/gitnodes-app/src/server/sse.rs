...
use gitnodes_domain::{TargetKey, TargetRef};
...
/// _>>>` as in `gitnodes_storage`'s caches) rather than pulling in `dashmap`:
...
) -> Result<TargetConfig, gitnodes_domain::TargetRefError> {
