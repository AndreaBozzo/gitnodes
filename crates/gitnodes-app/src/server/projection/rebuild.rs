...
use gitnodes_domain::{BrainConfig, BrainError, Edge, Node, TargetKey, split_frontmatter};
use gitnodes_graph::{RawFile, build_graph, parse_file};
...
impl ProjectionSnapshot {
    pub(super) fn from_raw_files(raw_files: &[RawFile], config: &BrainConfig) -> Self {
        ...
        // the same parse. `split_frontmatter` is the same helper gitnodes-graph
