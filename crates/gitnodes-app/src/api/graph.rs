...
use gitnodes_domain::{BrainConfig, TargetRef};
...
#[server(LoadBrainGraph, "/api", endpoint = "load_gitnodes_graph")]
pub async fn load_gitnodes_graph() -> Result<(Vec<Node>, Vec<Edge>), ApiError> {
    use crate::server::session;
...
#[server(RefreshBrainGraph, "/api", endpoint = "refresh_gitnodes_graph")]
...
pub async fn list_accessible_targets() -> Result<Vec<AccessibleTarget>, ApiError> {
    ...
    let repos_url = gitnodes_domain::GithubClient::new(target).user_repos_url();
...
#[cfg(feature = "ssr")]
async fn branch_exists(http: &gitnodes_storage::GithubHttp, token: &str, url: &str) -> bool {
    http.get(url, token)
        .send()
        .await
...
#[server(
    ...
    "/api",
    endpoint = "load_gitnodes_graph_for_target"
)]
pub async fn load_gitnodes_graph_for_target(
    target: TargetRef,
