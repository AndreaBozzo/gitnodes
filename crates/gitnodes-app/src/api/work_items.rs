...
use gitnodes_domain::{ExternalWorkItemBinding, TargetRef, WorkItem, WorkItemKind, WorkItemState};
...
async fn load_work_item_comments_inner(
    target: TargetRef,
    brain_id: String,
) -> Result<Vec<WorkItemComment>, gitnodes_domain::BrainError> {
    use crate::server::session;
    use gitnodes_domain::ExternalWorkItemSystem;
...
) -> Result<WorkItemMutationResult, gitnodes_domain::BrainError> {
    use crate::server::session;
...
) -> Result<Option<WorkItem>, gitnodes_domain::BrainError> {
    use gitnodes_domain::WorkItemSystemOfRecord;
...
async fn apply_work_item_mutation_inner(
    ...
    publish_event: bool,
) -> Result<WorkItem, gitnodes_domain::BrainError> {
    let prepared = prepare_work_item_mutation(
        token,
        user,
        target,
...
struct PreparedWorkItemMutation {
    ...
    mutated: WorkItem,
    path: String,
    transaction: gitnodes_storage::GitTransaction,
}
...
async fn prepare_work_item_mutation(
    ...
    brain_id: String,
    mutation: WorkItemMutation,
) -> Result<PreparedWorkItemMutation, gitnodes_domain::BrainError> {
    ...
    let (front_raw, body) = gitnodes_domain::split_frontmatter(&content);
    ...
    let transaction = gitnodes_storage::GitTransaction::new(commit_msg, user, author_email)
    ...
    if publish_event && let Some(bus) = crate::server::sse::global() {
        let event = if mutation.is_binding() {
            crate::server::sse::BrainEvent::BindingUpdated {
                target: gitnodes_domain::TargetRef::from(target),
...
async fn sync_work_item_provider(
    ...
    use gitnodes_domain::{ExternalWorkItemSystem, WorkItemSystemOfRecord};
    ...
    let mut patch = gitnodes_storage::GithubIssuePatch::default();
    match mutation {
        WorkItemMutation::State(state) => {
...
pub(crate) async fn reconcile_provider_sync(
    ...
    let mutation = match kind {
        ...
        "binding" => return Ok(()),
        other => {
            return Err(gitnodes_domain::BrainError::other(format!(
...
async fn github_labels_for_state(
    ...
    item: &WorkItem,
    state: &WorkItemState,
) -> Result<Option<Vec<String>>, gitnodes_domain::BrainError> {
