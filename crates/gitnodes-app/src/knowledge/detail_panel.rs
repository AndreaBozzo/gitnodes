...
/// the panel to render.
type BacklinkGroups = Vec<(gitnodes_domain::NodeTypeSpec, Vec<(String, String)>)>;
...
pub fn DetailPanel(
    ...
    let blob_base = Memo::new(move |_| {
        let target: gitnodes_domain::TargetConfig = active_target.get_value().into();
        gitnodes_domain::GithubClient::new(target).blob_base()
    });
...
fn unknown_backlink_spec(
    fallback: &gitnodes_domain::NodeTypeSpec,
    node_type: &str,
) -> gitnodes_domain::NodeTypeSpec {
...
mod tests {
    ...
    use gitnodes_domain::{BrainConfig, NodeTypeSpec};
