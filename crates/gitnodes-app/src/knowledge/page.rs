...
use crate::api::{
    ...
    list_brain_files, load_brain_config_status, load_gitnodes_graph, refresh_gitnodes_graph,
    search_brain,
};
use crate::app::{GraphVersion, SyncStatusSignal};
use gitnodes_domain::{TargetRef, encode_path_segment};
...
pub fn KnowledgePage() -> impl IntoView {
    ...
    // an empty DB while load_gitnodes_graph's bootstrap is still in progress.
        ...
        |_| async {
            #[cfg(feature = "ssr")]
            {
                tokio::time::timeout(std::time::Duration::from_secs(10), async {
                    let graph = load_gitnodes_graph().await?;
        ...
        <button
            ...
            on:click=move |_| {
                ...
                leptos::task::spawn_local(async move {
                    match refresh_gitnodes_graph(target).await {
                        Ok(()) => {
