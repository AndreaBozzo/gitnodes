...
pub fn OrphanBanner(
    ...
    config: StoredValue<gitnodes_domain::BrainConfig>,
    ...
    let target_ref = use_context::<gitnodes_domain::TargetRef>();

    let config_url = Memo::new(move |_| {
        if let Some(target_ref) = target_ref.clone() {
            return gitnodes_domain::GithubClient::new((&target_ref).into()).config_blob_url();
        }
        app_config
            .and_then(|r| r.get())
            .and_then(|r| r.ok())
            .map(|c| gitnodes_domain::GithubClient::new(c.target).config_blob_url())
            .unwrap_or_default()
    });
