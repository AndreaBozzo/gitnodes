...
pub fn DetailBar(
    ...
    let current_id = create_memo(move |_| selected.get().or_else(|| hovered.get()));
        ...
        Some(id) => nodes.with_value(|ns| {
            ...
            let links: Vec<String> = edges.with_value(|es| {
                es.iter()
                    ...
                    .filter_map(|other| ns.iter().find(|m| m.id == other).map(|m| m.title.to_string()))
                    .collect()
            });
            ...
            view! {
                <div class="flex items-start gap-4">
                    <div class="w-2 h-2 rounded-full mt-2" style=format!("background:{}", n.node_type.accent())></div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-3 flex-wrap">
                            <span class="text-[10px] uppercase tracking-widest text-slate-500">{n.node_type.label()}</span>
                            <h3 class="text-sm font-semibold text-slate-100 truncate">{n.title}</h3>
                            <div class="flex gap-1">
                                {n.tags.iter().map(|t| view! {
                        ...
                        <p class="text-[12px] text-slate-400 mt-1 leading-relaxed">{n.summary}</p>
